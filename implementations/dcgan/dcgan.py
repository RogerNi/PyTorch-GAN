...
parser.add_argument("--check_distribution", type=int, default=1, help="interval between checking the distribution of samples generated by the generator")
parser.add_argument("--fid_batch_size", type=int, default=300, help="batch size for fid computation")
parser.add_argument("--num_samples_check_distribution", type=int, default=1000, help="number of samples used to check the distribution of samples generated by the generator")
...
model = ConvNet(10).to("cuda" if cuda else "cpu")
...
for epoch in tqdm(range(opt.n_epochs)):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        if batches_done % opt.fid_freq == 0:
            ...
            gen_score = fid_score.calculate_fid_given_paths([mnist_gen_image_save_path, mnist_image_save_path], batch_size=opt.fid_batch_size, device='cuda', dims=2048)
            ...
            for i in tqdm(range(generated_samples.shape[0])):
                save_image(generated_samples[i], os.path.join(mnist_gen_image_save_path, f'{i}.png'), normalize=True, value_range=(-1., 1.))
            comb_score = fid_score.calculate_fid_given_paths([mnist_gen_image_save_path, mnist_image_save_path], batch_size=opt.fid_batch_size, device='cuda', dims=2048)
        ...
        if batches_done % opt.check_distribution == 0:
            ...
            for images in gen_imgs:
                ...
                count[predicted] += 1
            print("-- check_distribution:",count)
